...
void MenuScreen::create_grind_mode_page(lv_obj_t* parent) {
    ...
    // Time mode scale usage: display only - the grind is always driven by the clock
    create_description_label(parent, "Time mode: tare on start and show live weight. The grind always runs on time only, so a failed tare or broken load cell never stops it.");
    create_toggle_row(parent, "Scale", &time_scale_toggle);
    ...
    if (time_scale_toggle) {
        lv_obj_add_event_cb(time_scale_toggle, EventBridgeLVGL::dispatch_event, LV_EVENT_VALUE_CHANGED,
                           reinterpret_cast<void*>(static_cast<intptr_t>(ET::TIME_SCALE_TOGGLE)));
    }
...
void MenuScreen::update_grind_mode_toggles() {
    ...
    bool time_use_scale = true;  // Default to standard (scale-assisted) time mode
    if (hardware_manager) {
        ...
        if (main_prefs) {
            ...
            time_use_scale = main_prefs->getBool(GrindController::PREF_KEY_TIME_USE_SCALE, true);
        }
    }

    if (time_scale_toggle) {
        if (time_use_scale) {
            lv_obj_add_state(time_scale_toggle, LV_STATE_CHECKED);
        } else {
            lv_obj_clear_state(time_scale_toggle, LV_STATE_CHECKED);
        }
    }
