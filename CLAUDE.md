...
- **GrindController**: 9-phase state machine with predictive flow control, 10 pulse corrections, mechanical instability detection, and time mode additional pulses. Predictive stop uses a per-profile **learned coast time** (NVS keys `coast0..2`, EWMA alpha 0.125 - about an 8-grind average, clamped 0.05-1.5s) measured from the first settle after motor stop. The observation MUST divide by the same flow figure the prediction multiplied by (`coast_prediction_flow_gps`) or the units do not cancel and every learned coast comes out short - it previously used the 95th percentile against a mean and read ~25% low, which overshot persistently and could not self-correct. The prediction is inflated by `GRIND_COAST_SAFETY_FACTOR` so the grind lands light and a pulse tops it up, because coffee in the chute can be added and coffee in the cup cannot. Learning accepts overshoots and undershoots - only the measurement being spoiled (disturbed scale, out of range) or an aborted grind rejects it. Pulse correction converges instead of firing when the remaining error falls below `GRIND_PULSE_MIN_DELIVERY_G`, expressed as a weight rather than a duration so the worst-case undershoot does not scale with flow rate
...
**Diagnostics (`generate_diagnostic_report`, pulled over BLE):** carries `[CRASH DUMP]` (placed first, so a report that fails partway still shows why), `[MEMORY]`, `[STACK HEADROOM]`, `[ALGORITHM STATE]`, `[COAST MODEL]` with the last 10 observations and their accept/reject reason, `[RECENT ERRORS]`, and `[BOOT LOG]`.
