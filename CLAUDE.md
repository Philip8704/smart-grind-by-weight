...
- **Automation**: Start on Cup and Return on Removal toggles, plus a **Min. weight** dropdown (100g steps) that is the auto-start trigger itself - the grind begins when the scale settles above it. 0/Off disables auto-start entirely. Re-arms only after the weight drops 100g below the threshold and stays there 2s. Weight is measured against the empty-cradle reference stored at calibration (`hx_empty`), NOT the working tare - a grind zeroes with the portafilter in place, so the tare says nothing about what is on the scale. Steadiness uses peak-to-peak range over the window (`USER_AUTO_GRIND_STABLE_RANGE_G`), NOT `is_settled()`: that is 0.010g std dev, right for weighing a dose but far too tight for a 600g portafilter, and every brief excursion restarted the dwell so the trigger fired erratically
